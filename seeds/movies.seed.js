...
const MONGO_URI = 'mongodb://localhost:27017/express-movies'
//Aquí es cuándo le ponemos el nombre a la colección, no?

mongoose.connect(MONGO_URI)
 .then(response => console.log(`Connected to the database ${response.connection.name}`))
  //El console.log solamente lo hace cuando hacemos npm run seed, no?
  .then(() => {
    return Movie.create(movies)
  })
  .then(createdMovies => console.log(`Inserted ${createdMovies.length} shows in the database`))
